...
class Manus(Node):
    ...
    def run(self):
        ...
        while rclpy.ok():
            ...
            ik_keypoints = np.stack([keypoints[4], keypoints[8], keypoints[12], keypoints[16]])
            ...
            hand_right_msg = JointState()
            hand_right_msg.header.stamp = now
            hand_right_msg.name = ALLEGRO_JOINT_NAMES
            hand_right_msg.position = current_target.tolist()
            self.right_pub.publish(hand_right_msg)
...
def main(args=None):
    ...
    parser.add_argument('--model', default='krish_right', type=str)
    pargs = parser.parse_args()

    rclpy.init(args=args)

    human_ik = geort.load_model(pargs.model, epoch=100) 
